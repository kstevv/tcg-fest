...
const LEFT: Tier = {
  ...
  cta: { label: "GET BOTH", href: "/tickets" },
...
const RIGHT: Tier = {
  ...
  term: "for 8 months",
  finePrint: "",
  cta: { label: "GET IT NOW", href: "/tickets" },
...
export default function TicketTiers() {
  return (
    <section className="relative isolate" aria-labelledby="ticket-hero-heading">
      ...
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(80%_50%_at_50%_100%,rgba(0,0,0,.35),transparent_70%)]" />

      {/* MATCH Experience width/padding */}
    ...
    <div className="rounded-2xl bg-gradient-to-b from-[var(--color-lessdark)_20%] to-[rgba(19,19,19,0.4)] ring-1 ring-white/10 backdrop-blur shadow-2xl px-5 sm:px-6 py-5 sm:py-6">
      ...
      <div className="flex items-center gap-2">
        {tier.badges?.map((b, i) => (
          <span
            key={i}
            className="inline-flex h-8 items-center rounded-full bg-neutral-800 px-3 text-xs font-bold uppercase tracking-wide text-white/90 ring-1 ring-white/15"
          >
            {b.label}
          </span>
        ))}
      ...
      <div className="mt-3">
        ...
        <div className="text-emerald-400 text-xs font-semibold mt-1">{tier.subtitle}</div>
      </div>

      {/* Pricing block */}
      <div className="mt-4 text-white">
        <div className="flex items-baseline gap-2">
          {tier.oldPrice && <span className="text-sm text-white/60 line-through">{tier.oldPrice}</span>}
          <span className="text-2xl sm:text-3xl font-extrabold">{tier.newPrice}</span>
          <span className="text-base sm:text-lg font-semibold">,</span>
        </div>
        <div className="text-base sm:text-lg font-extrabold -mt-1">{tier.term}</div>
      ...
      <div className="mt-5">
        <Link
          href={tier.cta.href}
          className="block w-full rounded-xl bg-[#D52EF5] px-5 py-3 text-center text-sm font-bold text-white shadow-[inset_0_-2px_0_rgba(0,0,0,.2)] ring-1 ring-black/20 hover:bg-[#2a7ef0] active:translate-y-[1px] transition"
        >
          {tier.cta.label}
        </Link>
      </div>
